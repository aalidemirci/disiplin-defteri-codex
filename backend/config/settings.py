"""Disiplin Defteri — Django ayarları (tek dosya, tek kullanıcılı masaüstü uygulama).
...
# ---------------------------------------------------------------------------
# `DD_DATA_DIR` env değişkeni ile geçersiz kılınabilir (Electron/PyInstaller
...
DATA_DIR = Path(os.environ.get("DD_DATA_DIR", str(BASE_DIR / "data")))
...
# yok). Üretim paketlemesinde `DD_SECRET_KEY` env ile geçersiz kılınabilir.
SECRET_KEY = os.environ.get(
    "DD_SECRET_KEY",
    "django-insecure-disiplin-defteri-yerel-masaustu-gelistirme-anahtari",
)
...
# ortamı docker-compose.yml'de DD_DEBUG=1 ile açar.
DEBUG = _bool_env("DD_DEBUG", False)
...
MIDDLEWARE = [
    ...
    # Opsiyonel açılış parolası kapısı (§6): parola kuruluysa ve kilit açılmadıysa
    # veri uçlarını 423 Locked ile keser. Parola kurulu değilse hiçbir şey yapmaz.
...
# (`desktop/main.py`) açılışta rastgele bir belirteç üretip `DD_SESSION_TOKEN`
...
if os.environ.get("DD_SESSION_TOKEN"):
...
# DD_FRONTEND_DIR'i doldurur; geliştirmede depodaki frontend/dist kullanılır.
FRONTEND_DIR = Path(os.environ.get("DD_FRONTEND_DIR", str(BASE_DIR.parent / "frontend" / "dist")))
...
# ---------------------------------------------------------------------------
# Medya (disiplin dosya ekleri — veri dizini altında)
# ---------------------------------------------------------------------------
MEDIA_ROOT = DATA_DIR / "media"

# Dosya eki yükleme sınırı (MB) — OYS varsayılanıyla aynı.
MAX_UPLOAD_SIZE_MB = int(os.environ.get("DD_MAX_UPLOAD_SIZE_MB", "20"))
...
REST_FRAMEWORK = {
    ...
    "EXCEPTION_HANDLER": "shared.exceptions.dd_exception_handler",
}
