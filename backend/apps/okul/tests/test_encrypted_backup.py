...
def test_veritabani_ramde_sifrelenerek_ddbak_uretilir(
    ...
    assert filename.endswith(".ddbak")
...
def test_indirme_ucu_sifreli_dosyayi_ek_olarak_dondurur() -> None:
    with mock.patch(
        ...
        return_value=(b"DDBAK-encrypted", "yedek.ddbak"),
    ):
        response = APIClient().post("/api/v1/backups/encrypted/")

    assert response.status_code == 200
    assert response["Content-Type"] == "application/octet-stream"
    assert "yedek.ddbak" in response["Content-Disposition"]
    assert isinstance(response, StreamingHttpResponse)
    content = cast(Iterable[bytes], response.streaming_content)
    assert b"".join(content) == b"DDBAK-encrypted"
