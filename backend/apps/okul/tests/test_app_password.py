"""Opsiyonel uygulama parolası (F5-D5, tasarım §6) — uçtan uca davranış testleri.

Buradaki en önemli iddia şudur: **parola konduktan sonra veritabanı dosyasında
TCKN/telefon/veli bilgisi DÜZ METİN olarak ARANAMAZ.** O yüzden testler ORM'e
güvenmez, ham SQL ile sütunu okur (`connection.cursor()`), aksi hâlde şifreli
alan kendini çözer ve test yalan söylerdi.
...
VELI_TELEFON = "0" + str(AILE_YILMAZ["anne_tel"])
...
def ogrenci_olustur(**fazlasi: Any) -> Student:
    veri: dict[str, Any] = {
        "tckn": TCKN,
        "first_name": str(AILE_YILMAZ["ogrenci_first"]),
        "last_name": str(AILE_YILMAZ["ogrenci_last"]),
        "student_number": "101",
        "class_level": 10,
        "class_section": "A",
        "guardian_name": str(AILE_YILMAZ["anne_ad_soyad"]),
        "guardian_phone": VELI_TELEFON,
        "guardian_address": "Menteşe / Muğla",
    }
    veri.update(fazlasi)
    return cast("Student", Student.objects.create(**veri))


def ham_satir(student_id: int) -> tuple[str, str, str, str]:
    """ORM'i BYPASS ederek sütunları olduğu gibi okur (şifre çözülmez)."""
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT tckn, guardian_name, guardian_phone, guardian_address "
            "FROM okul_student WHERE id = %s",
            [student_id],
        )
        satir = cursor.fetchone()
    return (str(satir[0]), str(satir[1]), str(satir[2]), str(satir[3]))
...
class TestEnable:
    def test_alanlar_dbde_duz_metin_olarak_bulunamaz(self) -> None:
        ogrenci = ogrenci_olustur()
        app_password.enable(password=PAROLA)

        tckn, veli_ad, telefon, adres = ham_satir(ogrenci.pk)
        assert TCKN not in tckn
        assert str(AILE_YILMAZ["anne_ad_soyad"]) not in veli_ad
        assert VELI_TELEFON not in telefon
        assert "Menteşe" not in adres
        # Fernet token'ı 'gAAAA' ile başlar — gerçekten şifrelenmiş.
        assert tckn.startswith("gAAAA")

    def test_ad_soyad_ve_sinif_kapsam_disidir(self) -> None:
        """Tasarım kararı: arama/listeleme alanları şifrelenMEZ (bkz. models.py)."""
        ogrenci = ogrenci_olustur()
        app_password.enable(password=PAROLA)
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT last_name, student_number, class_section FROM okul_student WHERE id = %s",
                [ogrenci.pk],
            )
            soyad, numara, sube = cursor.fetchone()
        assert soyad == AILE_YILMAZ["ogrenci_last"]
        assert numara == "101"
        assert sube == "A"
    ...
    def test_kilit_acikken_orm_duz_metin_dondurur(self) -> None:
        ...
        assert tazelenmis.tckn == TCKN
        assert tazelenmis.guardian_phone == VELI_TELEFON
        assert tazelenmis.guardian_address == "Menteşe / Muğla"

    def test_silinmis_ogrencinin_alanlari_da_sifrelenir(self) -> None:
        ogrenci = ogrenci_olustur()
        ogrenci.delete()  # soft-delete
        app_password.enable(password=PAROLA)
        tckn, *_ = ham_satir(ogrenci.pk)
        assert tckn.startswith("gAAAA")
    ...
    def test_gecis_oncesi_yedek_sifrelemeden_ONCE_alinir(
        ...
        Testlerde veritabanı bellek içidir; gerçek dosya kopyası burada
        alınamaz (bkz. `take_transition_backup` notu), o yüzden çağrı ANI
        gözlenir: yedek alınırken TCKN hâlâ düz metin olmalıdır.
        """
        ogrenci = ogrenci_olustur()
        anlar: list[str] = []
...
class TestUnlock:
    def test_dogru_parola_acar(self) -> None:
        ...
        assert Student.objects.get(pk=ogrenci.pk).tckn.startswith("gAAAA")

        app_password.unlock(password=PAROLA)
        assert app_password.is_locked() is False
        assert Student.objects.get(pk=ogrenci.pk).tckn == TCKN
...
class TestDisable:
    def test_alanlar_duz_metne_doner(self) -> None:
        ...
        tckn, veli_ad, telefon, adres = ham_satir(ogrenci.pk)
        assert tckn == TCKN
        assert veli_ad == AILE_YILMAZ["anne_ad_soyad"]
        assert telefon == VELI_TELEFON
        assert adres == "Menteşe / Muğla"
...
class TestInterruptedTransition:
    def test_sifreleme_yarida_kalirsa_acilista_tamamlanir(self) -> None:
        ...
        # geri alır (bu tuzak testi bir kez sessizce yanlış yeşile boyadı).
        with (
        ...
        assert ham_satir(ogrenci.pk)[0] == TCKN
    ...
    def test_yarim_sifreli_tablo_okunabilir_ve_tekrar_kosulabilir(self) -> None:
        ...
        duz = ogrenci_olustur(tckn="10000000214", student_number="102")
        app_password.enable(password=PAROLA)
        # İkinci satırı elle düz metne çevirerek "yarım geçiş" üret.
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE okul_student SET tckn = %s WHERE id = %s", ["10000000214", duz.pk]
            )

        okunan = {o.pk: o.tckn for o in Student.objects.all()}
        assert okunan[sifreli.pk] == TCKN  # şifreli satır çözüldü
        assert okunan[duz.pk] == "10000000214"  # düz satır olduğu gibi geldi
        ...
        assert Student.objects.get(pk=sifreli.pk).tckn == TCKN
...
# ---------------------------------------------------------------------------
# Eşleştirme / arama davranışı
# ---------------------------------------------------------------------------
@pytest.mark.django_db
class TestLookupBehaviour:
    def test_tckn_eslesmesi_sifreli_kipte_de_calisir(self) -> None:
        ogrenci = ogrenci_olustur()
        app_password.enable(password=PAROLA)
        # DB filtresi artık çalışmaz (blind index yok) — seçici Python tarafına düşer.
        assert Student.objects.filter(tckn=TCKN).first() is None
        assert selectors.find_student_by_tckn(TCKN) == ogrenci

    def test_kilitliyken_tckn_eslesmesi_hata_verir(self) -> None:
        ogrenci_olustur()
        app_password.enable(password=PAROLA)
        app_password.lock()
        with pytest.raises(ValueError, match="kilitli"):
            selectors.find_student_by_tckn(TCKN)

    def test_ad_aramasi_sifreli_kipte_bozulmaz(self) -> None:
        ogrenci_olustur()
        app_password.enable(password=PAROLA)
        bulunan = selectors.student_list(search="yilmaz")
        assert [o.tckn for o in bulunan] == [TCKN]
...
class TestSecurityApi:
    def test_durum_ucu(self, client: APIClient) -> None:
        ...
        assert "TCKN" in veri["protected_fields"]
...
class TestLockMiddleware:
    """Kilit kapısı (`apps.okul.lock_middleware`) — settings'e bağlandığında."""

    @pytest.fixture(autouse=True)
    def middleware_acik(self, settings: Any) -> None:
        settings.MIDDLEWARE = [
            *settings.MIDDLEWARE,
...
class TestManagementCommand:
    ...
    def test_resume_force_yeniden_kosar(self) -> None:
        ...
        self._kosu("enable", password=PAROLA)
        with connection.cursor() as cursor:
            cursor.execute("UPDATE okul_student SET tckn = %s WHERE id = %s", [TCKN, ogrenci.pk])
