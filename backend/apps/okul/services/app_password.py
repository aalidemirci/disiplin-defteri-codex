...
zarf sarmalama, şifreli alanlar) sağlar; bu modül BÜTÜN AKIŞI yönetir:
    ...
    ├── kdf parametreleri                ├── okul_student.tckn ....... token
    ├── parola: {tuz, sarmal(DEK)}       ├── okul_student.guardian_* . token
...
NEDEN GÜVENLİK DOSYASI VERİ DİZİNİNDE, DB'DE DEĞİL?
  * Yedekler (`backups/gunluk-*.ddbak`) X25519 + AES-256-GCM kapsayıcılarıdır.
...
verinin OKUNUR ve geçişin TAMAMLANABİLİR kalacağı şekilde kurulmuştur:
    ...
    1. Geçiş öncesi şifreli yedek (`pre-parola-*.ddbak`)
...
logger = logging.getLogger("disiplin_defteri.guvenlik")

# --- Dosya/dizin çözümü -----------------------------------------------------
STATE_FILE_NAME = "guvenlik.json"
# Testler ve taşınabilir kip için: verilirse güvenlik dosyasının/yedeklerin yeri.
ENV_SECURITY_DIR = "DD_SECURITY_DIR"
ENV_BACKUP_DIR = "DD_BACKUP_DIR"
...
def _archive_state() -> Path | None:
    ...
    Gerekçe: eski yedekler (`gunluk-*.ddbak`) hâlâ ESKİ anahtarla şifrelidir.
...
def take_transition_backup(label: str) -> Path | None:
    ...
    Masaüstü kabuğuyla aynı RAM-içi SQLite görüntüsü ve `.ddbak` şifreli kapsayıcı
...
def _rewrite_rows() -> int:
    ...
    for model, alanlar in encrypted_field_map():
        # Soft-delete edilmiş satırlar da kapsanır: silinmiş öğrencinin TCKN'si
