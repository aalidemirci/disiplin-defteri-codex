# Codex Bilgilendirme — Disiplin Defteri

> Bu dosya, projeyi ilk kez gören bir denetçi/ajan içindir. Amacı: kodu doğru
> okumanı sağlamak ve **kasıtlı kararları kusur sanmanı engellemek**. Depodaki
> yorumlar ve `docs/` kütükleri gerekçe taşır — bir şey tuhaf görünüyorsa önce
> yakınındaki yorumu ve tasarım dokümanının ilgili bölümünü oku.
>
...
1. **Kasıtlı olanı kusur sanma.** §5 "bulgu DEĞİL" listesini rapor yazmadan önce
...
5. **Tarih ve büyük harf iki gerçek tuzak.** §6'ya bak — bu projede en çok gerçek
...
yeşil koştu (24.07.2026). Sürüm: `VERSION` → `2026.7.0-dev` (CalVer).
...
.github/workflows/paketleme.yml
```
...
## 5. Bunlar bulgu DEĞİL — kasıtlı kararlar
...
### 6.5 Bayat önbellek / tazelenmeyen liste
...
## 7. Bozulmaması gereken yasal invariantlar
...
*Kabul edilmiş bedeller:* K1-K7 → §5 tablosunda karşılıkları var.
...
3. **`frontend/src/modules/disiplin/DisiplinDetayPage.tsx` (2355 satır) +
   ...
   yerler; §6.5 ve §6.6 sınıfları burada yaşıyor.
...
## 11. Envanter (kaba rakamlar)

| | |
|---|---|
| Backend Python (test dahil, migration hariç) | ~22.500 satır |
| desktop/ + packaging/ Python | ~4.700 satır |
| Frontend TS/TSX | ~26.900 satır, 122 dosya |
| Evrak şablonları | 25 HTML + ortak `base.html` + `_design.css` |
| Backend testleri | 470 test fonksiyonu (parametrizasyonla ~483 koşu) |
| desktop + packaging testleri | ~111 fonksiyon |
| Frontend testleri | 360 |
| Migration | `okul` 2, `disiplin` 1 |
| Git geçmişi | 27 commit, tek dal (`main`) |
...
| `README.md` | ⚠️ **Bayat** — "Faz 0 tamamlandı, desktop/ ve packaging/ henüz boş" diyor; gerçekte F5 bitti ve her ikisi de dolu. Güncel durum için tasarım dokümanının §4.9 faz tablosuna bak. |

---

## 13. Not

Codex `AGENTS.md` adlı dosyayı depo kökünde kendiliğinden yükler; bu dosya farklı
adda olduğu için oturuma elle tanıtılmalı ("önce `codex-bilgilendirme.md` dosyasını
oku"). Kalıcı çözüm istenirse, tek satırlık bir `AGENTS.md` bu dosyaya işaret
edebilir.
