...
const repository = process.env.GITHUB_REPOSITORY || "aalidemirci/disiplin-defteri-codex";
const token = process.env.GITHUB_TOKEN || "";
const output = resolve(process.cwd(), "website/release-data.json");
const releasePage = `https://github.com/${repository}/releases`;

const kinds = [
  [/^disiplin-defteri-.+-win64-setup\.exe$/i, "windows_installer"],
  [/^disiplin-defteri-.+-win64-portable\.zip$/i, "windows_portable"],
  [/^disiplin-defteri_.+_amd64\.deb$/i, "linux_deb"],
  [/^disiplin-defteri-.+-linux-x64\.tar\.gz$/i, "linux_archive"],
...
async function build() {
  const headers = { Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2026-03-10", "User-Agent": "disiplin-defteri-pages" };
  ...
  return {
    ...
    name: String(release.name || release.tag_name || "En yeni sürüm"),
    prerelease: Boolean(release.prerelease),
    published_at: release.published_at || "",
    release_url: release.html_url || releasePage,
    assets,
  };
}
