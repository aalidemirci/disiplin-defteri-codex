// Kurulum sihirbazı testi (F4-D4): dört adımın mutlu yolu + kapılar + hata durumu.
// `../okul/api` (kurulum + ders yılı + tatil uçlarının tek sınırı) vi.mock ile taklit
...
describe("KurulumPage", () => {
  ...
  it("tatil adımı: seed sonucu gösterilir, ara tatil uyarısı vardır", async () => {
    ...
    oapi.getSchoolConfig.mockResolvedValue({ ...BOS_CONFIG, school_name: "Deneme Lisesi" });
    oapi.seedHolidays.mockResolvedValue({ created: 12, skipped: 3 });
    const user = userEvent.setup();
    renderPage();

    // Tatil sayısı 0 → sihirbaz 3. adımdan açılır.
    expect(await screen.findByText("3. Tatiller")).toBeInTheDocument();
    expect(screen.getByText(/Ara tatiller/)).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /Tatilleri yükle/ }));
    await waitFor(() => expect(oapi.seedHolidays).toHaveBeenCalled());
    expect(await screen.findByText(/12 tatil eklendi, 3 kayıt zaten vardı/)).toBeInTheDocument();
  });

  it("tatil seed hatası hata bandına düşer", async () => {
    oapi.getSetupStatus.mockResolvedValue({
      ...BOS_DURUM,
      school_name: "Deneme Lisesi",
      has_active_school_year: true,
    ...
    oapi.seedHolidays.mockRejectedValue(
      new ApiError(400, "validation_error", "Aktif ders yılı yok; önce bir ders yılı oluşturun."),
    );
    const user = userEvent.setup();
    renderPage();

    await screen.findByText("3. Tatiller");
    await user.click(screen.getByRole("button", { name: /Tatilleri yükle/ }));
    expect(
      await screen.findByText("Aktif ders yılı yok; önce bir ders yılı oluşturun."),
    ).toBeInTheDocument();
  });
  ...
  it("son adım: sayımlar görünür, kurulum tamamlanınca panele yönlendirilir", async () => {
    ...
    expect(await screen.findByText("4. Kişiler")).toBeInTheDocument();
  ...
  it("4. adım sicil aktarımının kurulumdan SONRA yapıldığını söyler", async () => {
    oapi.getSetupStatus.mockResolvedValue({
      ...BOS_DURUM,
      school_name: "Deneme Lisesi",
      has_active_school_year: true,
