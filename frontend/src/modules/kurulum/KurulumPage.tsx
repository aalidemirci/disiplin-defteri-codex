// Kurulum sihirbazı (F4-D4) — programın İLK AÇILIŞ ekranı. Dört adımda kurumu
// çalışır hale getirir: okul kimliği (resmî evrak anteti) → ders yılı (dosya
// numaralama + tatil takvimi buna bağlıdır) → tatil takvimi (yasal süre hesabı)
// → kişi sicili (öğrenci/personel içe aktarma yönlendirmesi).
//
...
// Tüm uçlar `modules/okul/api.ts` (backend `apps/okul` sınırı) üzerinden çağrılır —
// sihirbaza özel bir istemci katmanı yoktur.
...
import type { HolidaySeedResult, SchoolYear, SetupStatus } from "../okul/api";
...
export default function KurulumPage() {
  ...
  // Tek zorunlu kapı: aktif ders yılı olmadan tatil seed'i ve dosya numaralama çalışmaz.
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="dd-page-header">
        <div>
          <h1 className="dd-page-title">Kurulum sihirbazı</h1>
          <p className="dd-page-description">
            Programı kullanmaya başlamadan önce dört adımı tamamlayın. Girdiğiniz her bilgiyi daha
      ...
      {loading ? (
        ...
        !loadError && (
          <>
            ...
            {adim === 2 && (
              <TatilAdimi tatilSayisi={status?.holiday_count ?? 0} onSeeded={durumTazele} />
            )}
            {adim === 3 && <KisilerAdimi status={status} />}
...
// ---------------------------------------------------------------------------
// 1. adım — Okul bilgileri (evrak anteti)
// ---------------------------------------------------------------------------
  ...
  return (
    <Card elevation={1} className="p-6">
      ...
      <p className="mt-1 text-body-medium text-on-surface-variant">
        Bu bilgiler tutanak, tebligat ve karar evrakının antetinde ve imza bloğunda kullanılır.
      </p>
...
// ---------------------------------------------------------------------------
// 2. adım — Ders yılı (dosya numaralama + tatil takvimi buna bağlı)
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="space-y-4">
      <Card elevation={1} className="p-6">
        ...
        <p className="mt-1 text-body-medium text-on-surface-variant">
          Dosya numaraları, tatil takvimi ve kurul kayıtları AKTİF ders yılına bağlanır. Aynı anda
      ...
      <Card elevation={1} className="p-6">
        ...
        <p className="mt-2 text-label-small text-on-surface-variant">
          Yarıyıl tatili iki tarih arasında kalır. Bu dönemler Onur Genel Kurulu ve aylık Onur
          Kurulu toplantılarının takibinde kullanılır.
        </p>
        {error && (
          <div className="mt-4">
...
// ---------------------------------------------------------------------------
// 3. adım — Tatil takvimi (yasal süre hesabı iş günü sayar)
// ---------------------------------------------------------------------------
...
function TatilAdimi({
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [sonuc, setSonuc] = useState<HolidaySeedResult | null>(null);
  const snackbar = useSnackbar();
  ...
  const yukle = async () => {
    ...
    setError(null);
    try {
      const r = await okulApi.seedHolidays();
      setSonuc(r);
      snackbar.success(`${r.created} tatil eklendi.`);
      await onSeeded();
    } catch (e) {
      setError(hataMesaji(e, "Tatiller yüklenemedi."));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Card elevation={1} className="p-6">
      <p className="text-title-medium text-on-surface">3. Tatiller</p>
      <p className="mt-1 text-body-medium text-on-surface-variant">
        Yasal süreler iş günü üzerinden hesaplanır; resmî tatiller ve dini bayramlar takvime
        yüklenmezse süre uyarıları yanlış çıkar. Aktif ders yılı için tek tuşla yükleyin.
      </p>
      <p className="mt-2 text-body-medium text-on-surface-variant">
        Kayıtlı tatil sayısı: <span className="text-on-surface">{formatNumber(tatilSayisi)}</span>
      </p>

      <div className="mt-4 space-y-3">
        <BilgiBandi ikon="info">
          Dini bayram tarihleri TAHMİNİdir (takvimde &quot;tahmini&quot; işaretiyle görünür).
      ...
      {sonuc && (
        <p className="mt-4 text-body-medium text-on-surface">
          {formatNumber(sonuc.created)} tatil eklendi, {formatNumber(sonuc.skipped)} kayıt zaten
          vardı (atlandı).
        </p>
      )}
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-body-small text-on-surface-variant">
          Bu adımı atlayabilirsiniz; tatilleri sonra da ekleyebilirsiniz.
        </p>
        <Button icon="event_repeat" onClick={() => void yukle()} disabled={busy}>
          {busy ? "Yükleniyor…" : "Tatilleri yükle"}
        </Button>
      </div>
    </Card>
  );
...
// ---------------------------------------------------------------------------
// 4. adım — Kişiler (öğrenci/personel sicili Kişiler ekranından aktarılır)
// ---------------------------------------------------------------------------
...
function KisilerAdimi({ status }: { status: SetupStatus | null }) {
  return (
    <Card elevation={1} className="p-6">
      <p className="text-title-medium text-on-surface">4. Kişiler</p>
      <p className="mt-1 text-body-medium text-on-surface-variant">
        Öğrenci ve personel listeleri, kurulum tamamlandıktan sonra{" "}
        <span className="text-on-surface">Kişiler</span> ekranından aktarılır: uygulamanın Excel
        dosyasını seçin ya da tabloyu panodan yapıştırın. Önce ÖNİZLEME alınır — bu adım hiçbir
        kayıt yazmaz, yalnız uyarıları listeler; sonuç uygunsa AKTAR ile kaydedilir.
      </p>

      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <SayimKutusu ikon="school" etiket="Öğrenci" deger={status?.student_count ?? 0} />
        <SayimKutusu ikon="badge" etiket="Personel" deger={status?.personnel_count ?? 0} />
        <SayimKutusu ikon="event_busy" etiket="Tatil" deger={status?.holiday_count ?? 0} />
      </div>
