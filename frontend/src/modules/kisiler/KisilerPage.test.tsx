...
const STUDENT: Student = {
  ...
  gender: "K",
  status: "ACTIVE",
  guardian_name: "Fatma Yılmaz",
  guardian_kinship: "ANNE",
  guardian_phone: "05550000001",
  guardian_phone2: "",
  guardian_address: "Merkez",
};
...
const PREVIEW_REPORT: StudentImportReport = {
  ...
  dry_run: true,
  warnings: [
    { row_number: 3, field: "guardian_phone", issue: "Telefon geçersiz", raw_value: "555" },
  ],
  skipped: [],
};
...
describe("KisilerPage — öğrenci listesi", () => {
  it("mutlu yol: liste yüklenir, TCKN listede GÖSTERİLMEZ (KVKK)", async () => {
    renderPage();
    expect(await screen.findByText("Ayşe Yılmaz")).toBeInTheDocument();
    expect(screen.getByText("10/A")).toBeInTheDocument();
    expect(screen.getByText("Fatma Yılmaz")).toBeInTheDocument();
...
describe("KisilerPage — öğrenci ekleme/düzenleme/silme", () => {
  ...
  it("yeni öğrenci: boş sayı/tarih alanları null olarak gönderilir", async () => {
  ...
  it("backend alan hatası (fields) ilgili alanın altına yazılır", async () => {
    okulApiMock.createStudent.mockRejectedValue(
      new ApiError(400, "validation_error", "Girdiğiniz bilgileri kontrol edin.", {
        tckn: ["Geçersiz TCKN (11 hane + doğrulama)."],
      }),
    );
    const user = userEvent.setup();
    renderPage();
    ...
    expect(
      await within(dialog).findByText("Geçersiz TCKN (11 hane + doğrulama)."),
    ).toBeInTheDocument();
    expect(within(dialog).getByText("Girdiğiniz bilgileri kontrol edin.")).toBeInTheDocument();
  });

  it("satıra tıklama düzenleme formunu açar; TCKN yalnız burada görünür", async () => {
    const user = userEvent.setup();
    renderPage();
    await screen.findByText("Ayşe Yılmaz");
...
describe("KisilerPage — personel sekmesi", () => {
  it("sekmeye geçince personel listesi yüklenir", async () => {
    const user = userEvent.setup();
    renderPage();
    await screen.findByText("Ayşe Yılmaz");
...
describe("KisilerPage — içe aktarma paneli", () => {
  it("yapıştırılan metin: önizleme raporu gösterilir, Aktar önizlemeden önce kapalıdır", async () => {
    ...
    expect(screen.getByText("Telefon geçersiz")).toBeInTheDocument();
