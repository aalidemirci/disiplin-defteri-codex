...
export default function ThemeSwitcher({ className = "" }: Props) {
  ...
  return (
    <button
      ...
      title={label}
      aria-pressed={isDark}
      className={`group relative inline-flex h-[var(--dd-control-height)] w-[var(--dd-control-height)] items-center justify-center overflow-hidden rounded-shape-sm text-on-surface-variant transition hover:bg-on-surface/8 hover:text-on-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${className}`}
    >
