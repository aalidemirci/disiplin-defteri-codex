...
def test_belirtec_yoksa_middleware_hic_yuklenmez(monkeypatch: pytest.MonkeyPatch) -> None:
    """Geliştirme/test koşusunda (`DD_SESSION_TOKEN` boş) middleware devre dışıdır."""
...
def test_baslik_adi_django_meta_bicimindedir() -> None:
    assert TOKEN_HEADER == "HTTP_X_DD_TOKEN"  # `X-DD-Token` başlığının META karşılığı


def test_baslik_ile_de_gecilebilir(guard: SessionTokenMiddleware) -> None:
    yanit = guard(RequestFactory().get("/api/v1/students/", headers={"x-dd-token": TOKEN}))
