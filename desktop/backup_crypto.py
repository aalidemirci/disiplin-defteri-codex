"""Disiplin Defteri yedek kapsayıcısı.
...
_PUBLIC_BYTES = 32
_LENGTH_BYTES = 4
_PRIVATE_INFO = b"DisiplinDefteri/backup/private/v1"
_ENVELOPE_INFO = b"DisiplinDefteri/backup/envelope/v1"
...
def embedded_recovery_metadata(container: bytes) -> bytes:
    ...
    if len(container) <= fixed_len or not container.startswith(MAGIC):
        raise BackupCryptoError("Dosya geçerli bir Disiplin Defteri yedeği değil.")
