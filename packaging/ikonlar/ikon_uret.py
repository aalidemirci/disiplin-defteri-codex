...
`disiplin-defteri-logo.png`, ImageGen ile tasarlanıp alfa kanalı doğrulanmış ana
...
MASTER_SOURCE = OUTPUT_DIR / "disiplin-defteri-logo.png"
...
def generate() -> list[Path]:
    ...
    for size in PNG_SIZES:
        target = OUTPUT_DIR / f"disiplin-defteri-{size}.png"
        master.resize((size, size), Image.Resampling.LANCZOS).save(
            target, format="PNG", optimize=True
        )
        written.append(target)

    ico = OUTPUT_DIR / "disiplin-defteri.ico"
