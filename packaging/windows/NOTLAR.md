...
| W3 | `WEASYPRINT_DLL_DIRECTORIES` paket kökünü göstermek yeterli (WeasyPrint ≥60) | `rthook_dd.py` | aynı hata; alternatif `os.add_dll_directory()` |
...
| W6 | Inno Setup 6.3+ `ArchitecturesAllowed=x64compatible` destekliyor | `disiplin-defteri.iss` | derleme hatası → `x64` yazılır (6.2 ve öncesi) |
| W7 | `compiler:Languages\Turkish.isl` Inno kurulumunda mevcut | `disiplin-defteri.iss` | derleme hatası → dosya Inno deposundan indirilip eklenir |
| W8 | `PrivilegesRequired=lowest` ile `{autopf}` = `%LOCALAPPDATA%\Programs` | `disiplin-defteri.iss` | kurulum `Program Files`e gitmeye çalışıp yetki ister |
...
4. Kurulum paketini **yönetici olmayan** bir hesapta çalıştır → hiç UAC
   istemesin, `%LOCALAPPDATA%\Programs\Disiplin Defteri` altına kursun.
