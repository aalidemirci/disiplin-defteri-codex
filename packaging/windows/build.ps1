...
=============================================================================
 ...
 Çıktılar: dist\cikti\
   disiplin-defteri-<sürüm>-win64-setup.exe      (Inno, yönetici GEREKTİRMEZ)
   disiplin-defteri-<sürüm>-win64-portable.zip   (taşınabilir)
   SHA256SUMS.txt
=============================================================================
#>
[CmdletBinding()]
...
$AppDir     = Join-Path $PackageDir "disiplin-defteri"
$AppExe     = Join-Path $AppDir "disiplin-defteri.exe"
...
$env:DD_WITH_QT = if ($WithoutQt) { "0" } else { "1" }
$env:DD_DLL_DIR = $DllDir
& $PythonExe -m PyInstaller --noconfirm --clean --log-level WARN `
    --distpath $PackageDir --workpath $WorkDir `
    (Join-Path $Repo "packaging\pyinstaller\disiplin_defteri.spec")
...
$zip = Join-Path $Output "disiplin-defteri-$Version-win64-portable.zip"
...
if (-not $SkipInno) {
    ...
    & $iscc.Source `
        ...
        "/DSourceDir=$AppDir" `
        "/DOutputDir=$Output" `
        (Join-Path $Repo "packaging\windows\disiplin-defteri.iss")
