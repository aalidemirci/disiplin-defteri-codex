...
sınanacak. Linux yolu (`DD_FRONTEND_DIR`) debian:11 + debian:12 kurulum
...
2. **Fontconfig yapılandırması (yalnız Windows).** Windows'ta sistem fontconfig
   ...
   önbellek dizini (`%LOCALAPPDATA%\\DisiplinDefteri\\cache\\fontconfig`) ve
...
3. **Frontend dizini (her platform).** Derlenmiş SPA paket içinde
   `frontend/dist` altındadır; Django tarafı bu yolu `DD_FRONTEND_DIR`
   değişkeninden okur.

Kancadaki hiçbir hata programı durdurmaz: eksik font yapılandırması PDF'i
bozar ama disiplin kayıtlarına erişimi engellememeli. Sorunlar günlüğe değil
(günlük henüz kurulmadı) `DD_RTHOOK_UYARI` değişkenine yazılır.
"""
...
ENV_WARNING = "DD_RTHOOK_UYARI"
ENV_FRONTEND_DIR = "DD_FRONTEND_DIR"
...
def _cache_root() -> Path:
    ...
    try:
        ...
        return Path(fallback) / "DisiplinDefteri" / "cache"


def announce_frontend_dir(root: Path) -> None:
    """Derlenmiş SPA'nın yolunu `DD_FRONTEND_DIR` ile Django'ya bildirir."""
