...
"""PyInstaller spec — Disiplin Defteri (Windows + Linux ORTAK).

Kullanım (depo kökünden):

    pyinstaller --noconfirm --clean packaging/pyinstaller/disiplin_defteri.spec

Ortam değişkenleri:
    DD_WITH_QT=0   → PyQt5/QtWebEngine paketlenmez (yalnız `--autotest`/CI
                     doğrulaması için küçük ve hızlı derleme; pencere AÇILMAZ).
    DD_DLL_DIR     → (Windows) `dll_kapanisi.py` ile üretilmiş DLL klasörü.
...
WITH_QT = os.environ.get("DD_WITH_QT", "1").strip() not in {"0", "false", "no"}

DIST_NAME = "disiplin-defteri"
ENTRY = REPO / "packaging" / "pyinstaller" / "giris.py"
RUNTIME_HOOK = REPO / "packaging" / "pyinstaller" / "rthook_dd.py"
ICON = REPO / "packaging" / "ikonlar" / "disiplin-defteri.ico"
...
if WINDOWS:
    dll_dir = Path(os.environ.get("DD_DLL_DIR", str(REPO / "packaging" / "windows" / "dll")))
    ...
    for dll in dll_files:
        # Paket köküne konur; `rthook_dd.py` WEASYPRINT_DLL_DIRECTORIES ile gösterir.
