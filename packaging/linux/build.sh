...
#
# Ortam değişkenleri:
#   DD_WITH_QT=0  → PyQt5/QtWebEngine paketlenmez (hızlı doğrulama derlemesi;
#                   pencere açılmaz, yalnız `--autotest`/`--pdf-duman` çalışır)
#   DD_SKIP_PIP=1 → pip adımını atlar (bağımlılıklar zaten kurulu)
# =============================================================================
set -euo pipefail
...
DEB_SURUM="${SURUM/-/\~}"
DEB_ADI="disiplin-defteri_${DEB_SURUM}_amd64.deb"
TAR_ADI="disiplin-defteri-${SURUM}-linux-x64.tar.gz"
...
mkdir -p "$CIKTI"
DD_WITH_QT="$QT_ILE" pyinstaller \
    --noconfirm --clean --log-level WARN \
    --distpath "$PAKET_KOKU" \
    --workpath "$CALISMA" \
    "$DEPO/packaging/pyinstaller/disiplin_defteri.spec"

UYGULAMA="$PAKET_KOKU/disiplin-defteri/disiplin-defteri"
[ -x "$UYGULAMA" ] || { echo "HATA: çalıştırılabilir üretilmedi: $UYGULAMA" >&2; exit 1; }

bilgi "paket kişisel veri sızıntısı denetimi"
python "$DEPO/packaging/veri_sizintisi.py" "$PAKET_KOKU/disiplin-defteri"
...
DD_APP_HOME="$GECICI_VERI" "$UYGULAMA" --autotest
rm -rf "$GECICI_VERI"

# --- 6. .deb sargısı ---------------------------------------------------------
bilgi ".deb üretimi ($DEB_SURUM)"
mkdir -p "$DEB_AGACI/opt" "$DEB_AGACI/usr/bin" "$DEB_AGACI/usr/share/applications" "$DEB_AGACI/DEBIAN"
cp -a "$PAKET_KOKU/disiplin-defteri" "$DEB_AGACI/opt/disiplin-defteri"
ln -sf /opt/disiplin-defteri/disiplin-defteri "$DEB_AGACI/usr/bin/disiplin-defteri"
cp "$DEPO/packaging/linux/disiplin-defteri.desktop" "$DEB_AGACI/usr/share/applications/"

for boyut in 16 24 32 48 64 128 256; do
    hedef="$DEB_AGACI/usr/share/icons/hicolor/${boyut}x${boyut}/apps"
    mkdir -p "$hedef"
    cp "$DEPO/packaging/ikonlar/disiplin-defteri-${boyut}.png" "$hedef/disiplin-defteri.png"
done
...
TAR_AGACI="$TAR_KOKU/disiplin-defteri-${SURUM}"
mkdir -p "$TAR_AGACI"
cp -a "$PAKET_KOKU/disiplin-defteri" "$TAR_AGACI/uygulama"
...
cp "$DEPO/packaging/linux/disiplin-defteri.desktop" "$TAR_AGACI/"
mkdir -p "$TAR_AGACI/ikonlar"
cp "$DEPO/packaging/ikonlar/disiplin-defteri-"*.png "$TAR_AGACI/ikonlar/"
tar -czf "$CIKTI/$TAR_ADI" -C "$TAR_KOKU" "disiplin-defteri-${SURUM}"
